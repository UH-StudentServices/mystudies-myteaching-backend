...
package fi.helsinki.opintoni.web.rest.privateapi.profile.jobsearch;
...
@RestController
@RequestMapping(
    value = RestConstants.PRIVATE_API_V1_PROFILE + "/{profileId:" + RestConstants.MATCH_NUMBER + "}/jobsearch",
...
public class PrivateJobSearchResource extends AbstractResource {
    ...
    public ResponseEntity<JobSearchDto> insert(@UserId Long userId,
        ...
        return response(jobSearchService.insert(profileId, jobSearchDto));
    }

    @RequestMapping(method = RequestMethod.DELETE)
    public ResponseEntity<Void> insert(@UserId Long userId,
                                       @PathVariable Long profileId) {
        jobSearchService.delete(profileId);
