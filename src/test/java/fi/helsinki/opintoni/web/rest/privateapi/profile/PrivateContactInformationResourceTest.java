...
import fi.helsinki.opintoni.web.rest.privateapi.profile.contactinformation.UpdateSomeLink;
...
public class PrivateContactInformationResourceTest extends AbstractProfileResourceTest {
    ...
    public void thatSomeLinksAreDeleted() throws Exception {
        persistSomeLink(2L);
        assertThat(someLinkRepository.findByProfileId(2L).isEmpty()).isFalse();
        ...
        assertThat(someLinkRepository.findByProfileId(2L).isEmpty()).isTrue();
    }

    private void persistSomeLink(Long profileId) {
        SomeLink someLink = new SomeLink();
        someLink.profile = new Profile();
        someLink.profile.id = profileId;
