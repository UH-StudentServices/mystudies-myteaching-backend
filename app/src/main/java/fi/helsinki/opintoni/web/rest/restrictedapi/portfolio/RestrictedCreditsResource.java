...
@RestController
@RequestMapping(
    value = RestConstants.RESTRICTED_API_V1 + "/portfolio/{portfolioId:" + RestConstants.MATCH_NUMBER + "}/credits",
    produces = WebConstants.APPLICATION_JSON_UTF8)
@PublicVisibility(PortfolioComponent.CREDITS)
public class RestrictedCreditsResource extends AbstractResource {

    private final CreditsService creditsService;

    @Autowired
    public RestrictedCreditsResource(CreditsService creditsService) {
        this.creditsService = creditsService;
    }

    @RequestMapping(method = RequestMethod.GET)
    public ResponseEntity<CreditsDto> findByPortfolioId(@PathVariable Long portfolioId) {
