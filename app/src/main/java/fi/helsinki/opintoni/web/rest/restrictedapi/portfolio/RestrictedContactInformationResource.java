...
@RestController
@RequestMapping(value = RESTRICTED_API_V1 + "/portfolio/{portfolioId:" + MATCH_NUMBER + "}/contactinformation",
    produces = WebConstants.APPLICATION_JSON_UTF8)
@PublicVisibility(PortfolioComponent.CONTACT_INFORMATION)
public class RestrictedContactInformationResource extends AbstractResource {

    private final ContactInformationService contactInformationService;

    @Autowired
    public RestrictedContactInformationResource(ContactInformationService contactInformationService) {
        this.contactInformationService = contactInformationService;
    }

    @RequestMapping(method = RequestMethod.GET)
    public ResponseEntity<ContactInformationDto> findByPortfolioId(@PathVariable Long portfolioId) {
