...
@RestController
@RequestMapping(
    value = RestConstants.RESTRICTED_API_V1 + "/portfolio/{portfolioId:" + RestConstants.MATCH_NUMBER + "}/favorites",
    produces = WebConstants.APPLICATION_JSON_UTF8)
@PublicVisibility(PortfolioComponent.FAVORITES)
public class RestrictedPortfolioFavoriteResource extends AbstractResource {
    private final PortfolioFavoriteService portfolioFavoriteService;

    @Autowired
    public RestrictedPortfolioFavoriteResource(PortfolioFavoriteService portfolioFavoriteService) {
