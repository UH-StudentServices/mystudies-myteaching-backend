...
@RestController
@RequestMapping(value = RESTRICTED_API_V1 + "/portfolio/{portfolioId:" + MATCH_NUMBER + "}/summary",
    produces = WebConstants.APPLICATION_JSON_UTF8)
@PublicVisibility(PortfolioComponent.STUDIES)
public class RestrictedSummaryResource extends AbstractResource {

    private final PortfolioService portfolioService;

    @Autowired
    public RestrictedSummaryResource(PortfolioService portfolioService) {
        this.portfolioService = portfolioService;
    }

    @RequestMapping(method = RequestMethod.GET)
    public ResponseEntity<SummaryDto> get(@PathVariable Long portfolioId) {
