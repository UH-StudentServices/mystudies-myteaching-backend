...
@RestController
@RequestMapping(
    value = RestConstants.RESTRICTED_API_V1 + "/portfolio/{portfolioId:" + RestConstants.MATCH_NUMBER + "}/jobsearch",
    produces = WebConstants.APPLICATION_JSON_UTF8)
@PublicVisibility(PortfolioComponent.WORK_EXPERIENCE)
public class RestrictedJobSearchResource extends AbstractResource {

    private final JobSearchService jobSearchService;

    @Autowired
    public RestrictedJobSearchResource(JobSearchService jobSearchService) {
        this.jobSearchService = jobSearchService;
    }

    @RequestMapping(method = RequestMethod.GET)
    public ResponseEntity<JobSearchDto> findByPortfolioId(@PathVariable Long portfolioId) {
